...
public class DiscoverServiceImpl implements DiscoverService {
    ...
    private void checkServiceHealthStatus(ServiceVO serviceVO) {
        ...
        serviceVO.getInstances().forEach(instanceVO -> {
            updateInstanceHealthStatus(instanceVO);
            instanceStatus.add(instanceVO.getStatus().toString());
        });

        //If all of them are normal, set service status to ok
        if (instanceStatus.stream().allMatch(InstanceHealth.NORMAL.toString()::equals)) {
            serviceVO.setServiceStatus(ServiceStatus.OK);
        }
        //If none are normal, set service status to error
        else if (!instanceStatus.contains(InstanceHealth.NORMAL.toString())) {
            serviceVO.setServiceStatus(ServiceStatus.ERROR);
        }
        //If at least one normal and rest are timing out/slow/error, set service status to warning
        else {
